	...
	<div class="max-w-4xl mx-auto px-6 py-14">
		...
		{#if meta.affiliation?.department || meta.affiliation?.institution}
			<p class="text-slate-500 text-sm mt-1.5">
				{[meta.affiliation.department, meta.affiliation.institution].filter(Boolean).join(' · ')}
			</p>
		{/if}
		...
		<div class="flex flex-wrap gap-x-6 gap-y-2 mt-6">
			{#if meta.contact?.email}
				<a
					...
					class="flex items-center gap-2 text-slate-400 hover:text-sky-400 transition-colors text-sm"
				>
					<svg
			...
			{#if meta.contact?.website}
				<a
					href={meta.contact.website}
					target="_blank"
				...
				>
					...
					{meta.contact.website.replace(/^https?:\/\//, '')}
				</a>
			{/if}

			{#if meta.links?.github}
				<a
					href="https://github.com/{meta.links.github}"
					target="_blank"
					rel="noopener noreferrer"
					class="flex items-center gap-2 text-slate-400 hover:text-sky-400 transition-colors text-sm"
				>
			...
			{#if meta.links?.linkedin}
				<a
					href="https://linkedin.com/in/{meta.links.linkedin}"
					target="_blank"
					rel="noopener noreferrer"
					class="flex items-center gap-2 text-slate-400 hover:text-sky-400 transition-colors text-sm"
				>
			...
			{#if meta.links?.scholar}
				<a
					href="https://scholar.google.com/citations?user={meta.links.scholar}"
					target="_blank"
					rel="noopener noreferrer"
					class="flex items-center gap-2 text-slate-400 hover:text-sky-400 transition-colors text-sm"
				>
			...
			{#if meta.links?.orcid}
				<a
					href="https://orcid.org/{meta.links.orcid}"
					target="_blank"
					rel="noopener noreferrer"
					class="flex items-center gap-2 text-slate-400 hover:text-sky-400 transition-colors text-sm"
				>
					<span class="text-xs font-bold leading-none">iD</span>
					{meta.links.orcid}
				</a>
			{/if}
		</div>
